...
describe("sharing helpers", () => {
  ...
  it("returns an existing share token", async () => {
  ...
  it("creates a new token when enabling share", async () => {
    let call = 0;
    const client = createMockClient({
      from: () => {
        call += 1;
    ...
    // uuid is random — spy update path by returning fixed token from second call
    const token = await enableStashSharing(client, "stash-1");
    expect(token).toBe("generated-token");
  });

  it("revokes sharing by clearing the token", async () => {
    const client = createMockClient({
      from: () => {
        const builder: Record<string, unknown> = {};
        const self = () => builder;
